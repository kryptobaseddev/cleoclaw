...
export function TaskCustomFieldsEditor({
  ...
  if (isLoading)
    return <p className="text-xs text-app-text-quiet">{loadingMessage}</p>;
  if (definitions.length === 0) {
    return <p className="text-xs text-app-text-quiet">{emptyMessage}</p>;
  }
      ...
      {definitions.map((definition) => {
        ...
        return (
          <div key={definition.id} className="space-y-1">
            <label className="text-[11px] font-semibold uppercase tracking-wide text-app-text-quiet">
            ...
            {definition.description ? (
              <p className="text-xs text-app-text-quiet">{definition.description}</p>
            ) : null}
          </div>
        );
      })}
