...
const statusTone = (value?: string | null) => {
  switch (value) {
    case "in_progress":
      return "bg-app-success-soft text-app-success border-emerald-200";
    case "review":
      return "bg-app-warning-soft text-app-warning border-amber-200";
    case "done":
...
const priorityTone = (value?: string | null) => {
  switch (value) {
    case "high":
      return "bg-app-danger-soft text-app-danger border-rose-200";
    case "low":
      return "bg-app-surface-muted text-app-text-muted border-app-border";
    default:
...
function GroupChatMessageCard({ message }: { message: BoardGroupMemoryRead }) {
  return (
    <div className="rounded-2xl border border-app-border bg-app-surface-muted/60 p-4">
      ...
      {message.tags?.length ? (
        <div className="mt-3 flex flex-wrap gap-2 text-[11px] text-app-text-muted">
          {message.tags.map((tag) => (
            <span
              key={tag}
              className="rounded-full border border-app-border bg-app-surface px-2 py-0.5"
            >
              {tag}
            </span>
          ))}
...
export default function BoardGroupDetailPage() {
  ...
  return (
    <DashboardShell>
      ...
      <SignedIn>
        <DashboardSidebar />
        <main className="flex-1 overflow-y-auto bg-app-surface-muted">
            ...
            <div className="px-4 py-4 md:px-8 md:py-6">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <h1 className="mt-2 text-2xl font-semibold tracking-tight text-app-text">
                    {group?.name ?? "Group"}
                  </h1>
                  {group?.description ? (
                    ...
                    <p className="mt-2 text-sm text-app-text-quiet">
                      No description
                    </p>
                  )}
                </div>
              ...
              <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                <label className="inline-flex items-center gap-2 text-sm text-app-text-muted">
                  <input
                    type="checkbox"
                    className="h-4 w-4 rounded border-app-border text-blue-600"
                    checked={includeDone}
                ...
                <div className="flex items-center gap-2 text-sm text-app-text-muted">
                  <span className="text-app-text-quiet">Top tasks per board</span>
                    ...
                    {[0, 3, 5, 10].map((value) => (
                      <button
                        ...
                        className={cn(
                          ...
                          perBoardLimit === value
                            ? "bg-app-surface-strong text-white"
                ...
                <div className="flex flex-wrap items-center gap-2 text-sm text-app-text-muted">
                  <span className="text-app-text-quiet">Agent pace</span>
                  <div className="flex flex-wrap items-center gap-1 rounded-lg border border-app-border bg-app-surface p-1">
                  ...
                  <input
                    ...
                    className={cn(
                      "h-8 w-20 rounded-md border bg-app-surface px-2 text-xs text-app-text shadow-card",
                      heartbeatEvery
                        ? "border-app-border"
                  ...
                  <select
                    ...
                    className={cn(
                      "h-8 rounded-md border border-app-border bg-app-surface px-2 text-xs text-app-text shadow-card",
                      !canManageHeartbeat && "opacity-60 cursor-not-allowed",
                    )}
                    disabled={!canManageHeartbeat}
                  >
          ...
          <div className="p-4 md:p-8">
            <div className="space-y-6">
              {heartbeatApplyError ? (
                <div className="rounded-xl border border-rose-200 bg-app-danger-soft p-4 text-sm text-app-danger shadow-card">
                  {heartbeatApplyError}
                </div>
              ) : null}
              ...
              {snapshotQuery.isLoading ? (
                <div className="rounded-xl border border-app-border bg-app-surface p-6 text-sm text-app-text-muted shadow-card">
                ...
                <div className="grid gap-6 lg:grid-cols-2">
                  {boards.map((item) => (
                    <div
                      key={item.board.id}
                      className="overflow-hidden rounded-xl border border-app-border bg-app-surface shadow-card"
                    >
                        ...
                        <div className="flex items-start justify-between gap-4">
                          <div className="min-w-0">
                            <Link
                              ...
                              title="Open board"
                            >
                              <p className="truncate text-sm font-semibold text-app-text group-hover:text-blue-600">
                                {item.board.name}
                              </p>
                              <ArrowUpRight className="h-4 w-4 text-app-text-quiet group-hover:text-blue-600" />
                            </Link>
                          ...
                          <div className="flex flex-wrap items-center justify-end gap-2 text-xs">
                            <span className="rounded-full border border-app-border bg-app-surface-muted px-2 py-0.5 text-app-text-muted">
                              Inbox {safeCount(item, "inbox")}
                            </span>
                            <span className="rounded-full border border-emerald-200 bg-app-success-soft px-2 py-0.5 text-app-success">
                              In progress {safeCount(item, "in_progress")}
                            </span>
                            <span className="rounded-full border border-amber-200 bg-app-warning-soft px-2 py-0.5 text-app-warning">
                            ...
                            {item.tasks.map((task) => (
                              <li key={task.id}>
                                <Link
                                  ...
                                  className="block rounded-lg border border-app-border bg-app-surface-muted/40 p-3 transition hover:border-blue-200 hover:bg-blue-50/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-app-accent focus-visible:ring-offset-2"
                                ...
                                >
                                  ...
                                  <div className="mt-2 flex flex-wrap items-center justify-between gap-2 text-xs text-app-text-muted">
                                    ...
                                    <p className="font-mono text-[11px] text-app-text-quiet">
                                      {task.id}
                                    </p>
                                  </div>
                                </Link>
      ...
      {isChatOpen || isNotesOpen ? (
        <div
          className="fixed inset-0 z-40 bg-app-surface-strong/20"
          onClick={() => {
            setIsChatOpen(false);
            setChatError(null);
            setIsNotesOpen(false);
      ...
      <aside
        className={cn(
          "fixed right-0 top-0 z-50 h-full w-[560px] max-w-[96vw] transform border-l border-app-border bg-app-surface shadow-2xl transition-transform",
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className="flex flex-1 flex-col overflow-hidden px-6 py-4">
            ...
            <div className="flex-1 space-y-4 overflow-y-auto rounded-2xl border border-app-border bg-app-surface p-4">
              ...
              {chatHistoryQuery.isLoading && chatMessages.length === 0 ? (
                <p className="text-sm text-app-text-quiet">Loading…</p>
