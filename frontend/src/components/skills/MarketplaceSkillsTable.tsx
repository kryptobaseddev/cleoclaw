...
function riskPillClassName(risk: string | null | undefined) {
  ...
  switch (normalizedRisk) {
    case "safe":
    case "low":
      return "bg-[color:rgba(16,185,129,0.16)] text-app-success border border-emerald-200/70";
    case "medium":
    case "moderate":
      return "bg-[color:rgba(245,158,11,0.16)] text-app-warning border border-amber-200/70";
    case "elevated":
      return "bg-[color:rgba(245,158,11,0.16)] text-app-warning border border-amber-200/70";
    case "high":
    case "critical":
      return "bg-[color:rgba(244,63,94,0.16)] text-app-danger border border-rose-200/70";
    case "unknown":
      return "bg-[color:rgba(148,163,184,0.16)] text-app-text-muted border border-app-border/80";
    default:
...
export function MarketplaceSkillsTable({
    ...
    const baseColumns: ColumnDef<MarketplaceSkillCardRead>[] = [
      {
        ...
        cell: ({ row }) => (
          <div>
            ...
            <p
              className="mt-1 line-clamp-2 text-xs text-app-text-quiet"
        ...
        cell: ({ row }) => {
          ...
          return (
            <Link
              ...
              className="inline-flex items-center gap-1 text-sm font-medium text-app-text-muted hover:text-blue-600"
            >
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          if (!sourceHref) {
            return <span className="text-sm text-app-text-quiet">No source</span>;
          }
          ...
          return (
            <Link
              ...
              target="_blank"
              rel="noreferrer"
              className="text-sm font-medium text-app-text-muted hover:text-blue-600 hover:underline"
              title={sourceHref}
            >
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          if (installedOn.length === 0) {
            return <span className="text-sm text-app-text-quiet">-</span>;
          }
          return (
              ...
              {installedOn.map((gateway, index) => {
                ...
                return (
                  <span
                    ...
                    className="inline-flex items-center gap-1 text-sm text-app-text-muted"
                    title={gateway.name}
                  >
                    <Link
  ...
  return (
    <DataTable
      ...
      rowClassName="transition hover:bg-app-surface-muted"
