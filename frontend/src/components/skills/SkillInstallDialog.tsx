    ...
    <Dialog open={Boolean(selectedSkill)} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="mt-2 space-y-3.5">
          {isGatewayStatusLoading ? (
            <p className="text-sm text-app-text-quiet">Loading gateways...</p>
          ) : (
            gateways.map((gateway) => {
              ...
              return (
                <div
                  key={gateway.id}
                  className="flex items-center justify-between rounded-xl border border-app-border bg-app-surface p-4"
                >
                  <div>
          ...
          {gatewayStatusError ? (
            <p className="text-sm text-app-danger">{gatewayStatusError}</p>
          ) : null}
          {mutationError ? (
        ...
        <DialogFooter className="mt-6 border-t border-app-border pt-4">
          <Button
            variant="outline"
