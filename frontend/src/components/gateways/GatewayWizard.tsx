...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      className="ml-2 inline-flex shrink-0 items-center rounded p-1.5 text-slate-400 hover:bg-slate-200 hover:text-slate-600"
      aria-label="Copy"
    >
      {copied ? (
    ...
    <div className="flex items-center gap-2">
      {steps.map((step, i) => (
        <div key={step.key} className="flex items-center gap-2">
          {i > 0 ? (
            <div
              className={`h-px w-6 ${i <= currentIndex ? "bg-slate-900" : "bg-slate-200"}`}
            />
          ) : null}
          <div className="flex items-center gap-2">
            <span
            ...
            <span
              className={`text-sm font-medium ${i === currentIndex ? "text-slate-900" : "text-slate-400"}`}
            >
              {step.label}
            </span>
          </div>
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-slate-50">
      ...
      <div className="flex items-start gap-2 p-3">
        <pre className="flex-1 overflow-x-auto whitespace-pre-wrap font-mono text-xs leading-relaxed text-slate-700">
          {code}
        </pre>
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="rounded-lg border border-blue-100 bg-blue-50 p-3 text-xs text-blue-700">
            ...
            <div className="mt-1.5 flex items-center">
              <code className="rounded bg-blue-100 px-2 py-0.5 font-mono">
                grep token ~/.openclaw/openclaw.json
              </code>
              <CopyButton text='grep token ~/.openclaw/openclaw.json' />
            </div>
            <p className="mt-1.5">
              Look for <code className="rounded bg-blue-100 px-1 font-mono">gateway.auth.token</code> value.
              If you haven{"'"}t set one yet, run{" "}
              <code className="rounded bg-blue-100 px-1 font-mono">openclaw onboard</code> first.
            </p>
          </div>
        </div>
      </div>
      ...
      {state.connectionError ? (
        <p className="text-sm text-red-500">{state.connectionError}</p>
      ) : null}
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          <table className="w-full text-sm">
            <tbody className="divide-y divide-slate-100">
              <tr>
                <td className="py-2 pr-4 font-medium text-slate-600">Forward Port</td>
                <td className="py-2 font-mono">3011 <span className="text-slate-400">(CCMC frontend)</span></td>
              </tr>
              <tr>
                <td className="py-2 pr-4 font-medium text-slate-600">Websockets Support</td>
                <td className="py-2 font-semibold text-emerald-600">ON</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-4">
            <table className="w-full text-sm">
              <tbody className="divide-y divide-slate-100">
                <tr>
                  <td className="py-2 pr-4 font-medium text-slate-600">Domain Names</td>
                  <td className="py-2 font-mono text-slate-900">{domain}</td>
                </tr>
                <tr>
                  <td className="py-2 pr-4 font-medium text-slate-600">Forward Port</td>
...
function StepCreate({
  ...
  return (
    <div className="space-y-6">
      ...
      {state.createError ? (
        <p className="text-sm text-red-500">{state.createError}</p>
      ) : null}
    </div>
  );
}
